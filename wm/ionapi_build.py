#!/usr/bin/env python3
"""Package IonApiClient (client ION API / OAuth 2 par compte de service, sans gestion manuelle des jetons)
et package IonApiMock (faux serveur de jetons + faux M3 pour la démo). Construits par putNode via le serveur MCP.
...
DTN = "yyyyMMddHHmmss"   # même instant en nombre comparable (calculateDateDifference rend une valeur absolue)
S = lambda n: f"/{n};1;0"          # chaîne
O = lambda n: f"/{n};3;0"          # objet
...
def store_get(key_expr, target):
    """ionapi.store:get -> target (absent si la clé n'existe pas)."""
...
def body_to_string(target):
    """body/bytes -> chaîne (absente si pas de corps)."""
...
def token_request(args, comment):
    """POST form-urlencoded vers tokenUrl -> httpStatus, respJson."""
...
def take_token(source):
    return [json_parse("/respJson;1;0", "/resp;2;0"),
            ...
            branch("/expObj", ("$null", []), ("$default", [invoke("pub.string:objectToString", inp=[copy("/expObj;3;0", "/object;3;0")], out=[copy("/string;1;0", "/expiresInRaw;1;0"), delete("/string;1;0", "/object;3;0")])]),
                   comment="expires_in arrive en nombre JSON (Long) : conversion en chaîne, 3600 s par défaut"),
...
# ------------------------------------------------------------------ ionapi.store : coffre de mots de passe sortants (chiffré)
STORE_SET = service("ionapi.store:set", PKG, sig(field("key"), field("value")), sig(), [
    ...
    quiet(invoke("pub.security.outboundPasswords:removePassword", inp=[setv("/isInternal;1;0", "false")], out=[delete("/result;1;0", "/message;1;0", "/isInternal;1;0")]), comment="remplacement : suppression préalable ignorée si absente"),
...
], "Écrit une valeur (chaîne) dans le coffre de mots de passe sortants de l'IS, chiffrée au repos (clé -> valeur).")
...
LOG_EVENT = service("ionapi.token:logEvent", PKG, sig(field("alias"), field("event"), field("detail")), sig(), [
    ...
    branch_expr(("%len% > 20000", [
        ...
        json_string("/journal;2;0", "/newJson;1;0")]), comment="journal borné : repart du dernier événement au-delà de 20 Ko"),
...
], "Journal des événements de jetons (obtention, renouvellement, rejet) conservé dans le coffre.")
...
TOKEN_GET = service("ionapi.token:get", PKG, sig(field("alias"), field("forceRefresh")), sig(field("accessToken"), field("tokenSource"), field("expiresAt"), field("expiresIn"), field("apiBaseUrl")), [
    *try_catch([
    ...
    branch("/cfgJson", ("$null", [exit_("$parent", "FAILURE", "Accès ION API inconnu : %alias%")])),
    ...
    branch("/tokJson", ("$null", []), ("$default", [
        ...
        branch_expr(("%forceRefresh% != \"true\" && %tok/expiresAtNum% > %nowNum%", [
            ...
            invoke("pub.date:calculateDateDifference", inp=[copy("/now;1;0", "/startDate;1;0"), copy("/tok;2;0/expiresAt;1;0", "/endDate;1;0"), setv("/startDatePattern;1;0", DT), setv("/endDatePattern;1;0", DT)],
                   ...
                   comment="secondes restantes")]), comment="jeton encore valable (comparaison numérique des instants) ?"),
    ]), comment="jeton déjà connu pour cet accès ?"),
    branch("/valid",
           ("true", [mapstep(copy("/tok;2;0/access_token;1;0", "/accessToken;1;0"), copy("/tok;2;0/expiresAt;1;0", "/expiresAt;1;0"), copy("/remaining;1;0", "/expiresIn;1;0"), setv("/tokenSource;1;0", "cache"), comment="jeton en cache encore valable")]),
           ("$default", [
               branch_expr(("%tok/refresh_token% != $null && %forceRefresh% != \"true\"", [
                   quiet(token_request([("grant_type", "refresh_token", True), ("refresh_token", "/tok;2;0/refresh_token;1;0", False), ("client_id", "/cfg;2;0/clientId;1;0", False), ("client_secret", "/cfg;2;0/clientSecret;1;0", False)], "grant refresh_token"),
                         branch("/httpStatus", ("200", take_token("refresh"))),
                         mapstep(delete("/respJson;1;0", "/httpStatus;1;0")), comment="renouvellement par refresh token, échec ignoré (on repasse par le mot de passe)")])),
               branch("/newToken", ("$null", [
                   token_request([("grant_type", "password", True), ("username", "/cfg;2;0/saak;1;0", False), ("password", "/cfg;2;0/sask;1;0", False), ("client_id", "/cfg;2;0/clientId;1;0", False), ("client_secret", "/cfg;2;0/clientSecret;1;0", False), ("scope", "/cfg;2;0/scope;1;0", False)],
                                 "grant password : compte de service (saak / sask) + client id / secret"),
                   branch("/httpStatus", ("200", take_token("password")), ("$default", [exit_("$parent", "FAILURE", "Authentification refusée par le serveur de jetons (HTTP %httpStatus%) : %respJson%")])),
                   mapstep(delete("/respJson;1;0", "/httpStatus;1;0"))]), comment="pas de jeton : authentification par le compte de service"),
               ...
               invoke("ionapi.token:logEvent", inp=[copy("/tokenSource;1;0", "/event;1;0"), setv("/detail;1;0", "jeton obtenu, valable %expiresInRaw% s", variables=True)], out=[delete("/event;1;0", "/detail;1;0")]),
           ...
           ]), comment="cache valable ? sinon refresh, sinon mot de passe"),
    mapstep(delete("/cfg;2;0", "/tok;2;0", "/tokJson;1;0", "/now;1;0", "/nowNum;1;0", "/expiresAtNum;1;0", "/valid;1;0", "/remaining;1;0", "/tokenUrl;1;0", "/forceRefresh;1;0")),
        ...
        mapstep(delete("/cfg;2;0", "/tok;2;0", "/tokJson;1;0", "/cfgJson;1;0", "/now;1;0", "/nowNum;1;0", "/expiresAtNum;1;0", "/valid;1;0", "/remaining;1;0", "/tokenUrl;1;0", "/forceRefresh;1;0", "/newToken;1;0", "/newRefresh;1;0",
                       "/expiresInRaw;1;0", "/ttl;1;0", "/tokState;2;0", "/tokStateJson;1;0", "/respJson;1;0", "/respBytes;3;0", "/httpStatus;1;0", "/resp;2;0"), comment="aucun secret ne doit rester dans le pipeline"),
        exit_("$flow", "FAILURE", "%tokenError%"),
    ]),
], "Fournit un jeton d'accès valable pour un accès ION API : cache (coffre), sinon refresh token, sinon grant password avec le compte de service. Le flow appelant ne manipule jamais les identifiants ni l'expiration.")
...
API_CALL = service("ionapi.api:call", PKG, sig(field("alias"), field("method"), field("path"), field("body"), field("contentType")),
    ...
    mapstep(setv("/method;1;0", "get", overwrite=False), setv("/contentType;1;0", "application/json", overwrite=False), setv("/retried;1;0", "false"), comment="valeurs par défaut"),
    invoke("ionapi.token:get", out=[delete("/expiresAt;1;0", "/expiresIn;1;0")], comment="jeton (cache / refresh / mot de passe) sans exposer les identifiants"),
    mapstep(setv("/url;1;0", "%apiBaseUrl%%path%", variables=True)),
    http_api_call("appel ION API avec Authorization: Bearer"),
    body_to_string("/respBody;1;0"), mapstep(delete("/respBytes;3;0")),
    branch("/httpStatus", ("401", [
        invoke("ionapi.token:get", inp=[setv("/forceRefresh;1;0", "true")], out=[delete("/expiresAt;1;0", "/expiresIn;1;0", "/forceRefresh;1;0")], comment="401 : jeton révoqué ou expiré côté serveur, on se ré-authentifie"),
        mapstep(setv("/retried;1;0", "true")),
        invoke("ionapi.token:logEvent", inp=[setv("/event;1;0", "retry-401"), setv("/detail;1;0", "%method% %path% : 401, nouveau jeton puis nouvel essai", variables=True)], out=[delete("/event;1;0", "/detail;1;0")]),
        mapstep(delete("/respBody;1;0")), http_api_call("nouvel essai avec le nouveau jeton"), body_to_string("/respBody;1;0"), mapstep(delete("/respBytes;3;0"))]), comment="un seul nouvel essai sur 401"),
...
], "Appelle une ressource ION API (méthode, chemin relatif à la base de l'accès, corps optionnel) : jeton géré automatiquement, nouvel essai unique après un 401.")
...
SAVE_ALIAS = service("ionapi.admin:saveAlias", PKG,
    ...
    branch("/alias", ("$null", [exit_("$flow", "FAILURE", "Le nom de l'accès (alias) est requis")])),
    branch("/ionapiJson", ("$null", []), ("$default", [
        ...
        mapstep(copy("/ion;2;0/ti;1;0", "/tenant;1;0"), copy("/ion;2;0/ci;1;0", "/clientId;1;0"), copy("/ion;2;0/cs;1;0", "/clientSecret;1;0"), copy("/ion;2;0/saak;1;0", "/saak;1;0"), copy("/ion;2;0/sask;1;0", "/sask;1;0"),
                ...
                comment="lecture du fichier .ionapi : ti, ci, cs, saak, sask, iu, pu, ot")]), comment="fichier .ionapi fourni ?"),
    branch("/pu", ("$null", []), ("$default", [mapstep(setv("/tokenUrl;1;0", "%pu%%ot%", variables=True, overwrite=False), comment="URL du jeton = pu + ot")])),
    branch("/iu", ("$null", []), ("$default", [branch("/tenant", ("$null", [mapstep(setv("/apiBaseUrl;1;0", "%iu%", variables=True, overwrite=False))]), ("", [mapstep(setv("/apiBaseUrl;1;0", "%iu%", variables=True, overwrite=False))]),
                                                    ("$default", [mapstep(setv("/apiBaseUrl;1;0", "%iu%/%tenant%", variables=True, overwrite=False))]), comment="base des API = iu / tenant")])),
    branch_expr(("%tokenUrl% == $null || %apiBaseUrl% == $null || %clientId% == $null || %clientSecret% == $null || %saak% == $null || %sask% == $null || %tokenUrl% == \"\" || %apiBaseUrl% == \"\" || %clientId% == \"\" || %clientSecret% == \"\" || %saak% == \"\" || %sask% == \"\"",
                 [exit_("$flow", "FAILURE", "Configuration incomplète : il faut tokenUrl, apiBaseUrl, clientId, clientSecret, saak et sask (fichier .ionapi ou saisie manuelle)")])),
    now_str("/savedAt;1;0"),
    ...
    invoke("ionapi.token:logEvent", inp=[setv("/event;1;0", "config"), setv("/detail;1;0", "accès enregistré (%tokenUrl%)", variables=True)], out=[delete("/event;1;0", "/detail;1;0")]),
    mapstep(setv("/ok;1;0", "true"), setv("/message;1;0", "Accès enregistré ; les identifiants sont chiffrés dans le coffre de l'Integration Server"),
            delete("/cfg;2;0", "/cfgJson;1;0", "/ionapiJson;1;0", "/clientSecret;1;0", "/sask;1;0", "/saak;1;0", "/scope;1;0", "/pu;1;0", "/iu;1;0", "/ot;1;0", "/savedAt;1;0")),
], "Enregistre un accès ION API à partir du fichier .ionapi (ou de champs saisis) : les secrets vont dans le coffre chiffré de l'IS, rien d'autre à configurer.")
...
DELETE_ALIAS = service("ionapi.admin:deleteAlias", PKG, sig(field("alias")), sig(field("ok")), [
    ...
    invoke("ionapi.token:logEvent", inp=[setv("/event;1;0", "config"), setv("/detail;1;0", "accès supprimé")], out=[delete("/event;1;0", "/detail;1;0")]),
    mapstep(setv("/ok;1;0", "true")),
], "Supprime un accès et son jeton.")
    ...
    loop("/allKeys", None,
         ...
         branch("/pos", ("0", [
             ...
             mapstep(copy("/cfg;2;0/alias;1;0", "/entry;2;0/alias;1;0"), copy("/cfg;2;0/tenant;1;0", "/entry;2;0/tenant;1;0"), copy("/cfg;2;0/apiBaseUrl;1;0", "/entry;2;0/apiBaseUrl;1;0"),
                     ...
                     copy("/cfg;2;0/scope;1;0", "/entry;2;0/scope;1;0"), copy("/cfg;2;0/savedAt;1;0", "/entry;2;0/savedAt;1;0"), comment="jamais les secrets"),
         ...
         mapstep(delete("/pos;1;0")), comment="une entrée par clé ionapi.alias.*"),
    store_get("ionapi.events", "/evJson;1;0"),
...
], "Liste les accès configurés (sans les secrets), l'état de leur jeton et le journal des événements.")
...
TEST_ALIAS = service("ionapi.admin:testAlias", PKG, sig(field("alias")), sig(field("ok"), field("message"), field("tokenSource"), field("expiresIn"), field("tokenPreview"), field("elapsedMs")), [
    invoke("pub.date:currentNanoTime", out=[copy("/nanoTime;3;0", "/t0;3;0"), delete("/nanoTime;3;0")]),
    *try_catch([
        invoke("ionapi.token:get", inp=[setv("/forceRefresh;1;0", "true")], out=[delete("/forceRefresh;1;0", "/expiresAt;1;0", "/apiBaseUrl;1;0")], comment="authentification forcée"),
        ...
        mapstep(setv("/ok;1;0", "true"), setv("/message;1;0", "Jeton obtenu (%tokenSource%), valable %expiresIn% s", variables=True), delete("/accessToken;1;0")),
    ], [
    ...
    *elapsed_ms("/t0;3;0", "/elapsedMs;1;0"),
], "Teste un accès : obtient un jeton avec le compte de service et rend un diagnostic lisible.")

# ------------------------------------------------------------------ IonApiMock : faux serveur de jetons + faux M3
...
MOCK_TOKEN = service("ionapimock.sso:token", MOCK, sig(field("grant_type"), field("username"), field("password"), field("client_id"), field("client_secret"), field("refresh_token"), field("scope")), sig(), [
    mapstep(copy("/password;1;0", "/saskIn;1;0"), delete("/password;1;0"), comment="on libère le nom 'password' utilisé par le coffre"),
    store_get("ionapimock.ttl", "/ttl;1;0"),
    mapstep(setv("/ttl;1;0", "90", overwrite=False), comment="durée de vie des jeton émis (réglable, 90 s par défaut pour la démo)"),
    branch_expr(
        (f'%client_id% != "{CLIENT_ID}" || %client_secret% != "{CLIENT_SECRET}"', reject("401", '{"error":"invalid_client","error_description":"client id / secret incorrects"}')),
        (f'%grant_type% == "password" && %username% != "{SAAK}"', reject("401", '{"error":"invalid_grant","error_description":"compte de service inconnu (saak)"}')),
        (f'%grant_type% == "password" && %saskIn% != "{SASK}"', reject("401", '{"error":"invalid_grant","error_description":"secret du compte de service incorrect (sask)"}')),
        ('%grant_type% == "refresh_token"', [store_get("ionapimock.refresh.%refresh_token%", "/rt;1;0"),
                                             branch("/rt", ("$null", reject("401", '{"error":"invalid_grant","error_description":"refresh token inconnu"}')), ("$default", ISSUE))]),
        ('%grant_type% == "password"', ISSUE),
        ("$default", reject("400", '{"error":"unsupported_grant_type"}')),
        comment="serveur de jetons simulé : grants password et refresh_token"),
...
], "Faux serveur de jetons ION (grant password avec compte de service, refresh token, jetons à durée de vie courte pour montrer le renouvellement).")
...
def bearer_check():
    return [
        ...
        branch("/authHdr", ("$null", reject("401", '{"error":"invalid_token","error_description":"Authorization: Bearer manquant"}'))),
        ...
        branch("/expiresAt", ("$null", reject("401", '{"error":"invalid_token","error_description":"jeton inconnu"}'))),
        now_str("/now;1;0"),
        invoke("pub.date:calculateDateDifference", inp=[copy("/now;1;0", "/startDate;1;0"), copy("/expiresAt;1;0", "/endDate;1;0"), setv("/startDatePattern;1;0", DT), setv("/endDatePattern;1;0", DT)],
               out=[copy("/dateDifferenceSeconds;1;0", "/remaining;1;0"), delete("/dateDifferenceSeconds;1;0", "/dateDifferenceMinutes;1;0", "/dateDifferenceHours;1;0", "/dateDifferenceDays;1;0", "/startDate;1;0", "/endDate;1;0", "/startDatePattern;1;0", "/endDatePattern;1;0")]),
        branch_expr(("%remaining% <= 0", reject("401", '{"error":"expired_token","error_description":"jeton expiré"}'))),
...
], "Faux M3 : GetBasicData d'un client (exige un jeton Bearer valable).")
...
], "Règle la durée de vie des jetons émis par le faux serveur (démo du renouvellement).")
...
], "Durée de vie courante des jetons du faux serveur.")
...
def deploy_all(m):
    ...
    # le simulateur ION API est un serveur autonome (mock/ion_mock.py) : l'IS intercepte tout Authorization: Bearer entrant
...
def tests(m):
    try:
        ...
        sys.exit("simulateur ION API absent : lancer  python3 mock/ion_mock.py 8085 &")
    inv(m, "ionapi.store:set", {"key": "ionapi.selftest", "value": "coucou"})
    r = inv(m, "ionapi.store:get", {"key": "ionapi.selftest"}); assert r and r.get("value") == "coucou", "coffre KO"
    ...
    r = inv(m, "ionapi.api:call", {"alias": "demo", "method": "get", "path": M3_PATH + "?CUNO=C000042"}, 400); assert r and r.get("status") == "200", "appel KO"
    r = inv(m, "ionapi.api:call", {"alias": "demo", "path": M3_PATH + "?CUNO=C000043"}, 200); assert r and r.get("tokenSource") == "cache", "cache KO"
    inv(m, "ionapi.admin:listAliases", maxlen=500)
    print("--- jeton court (20 s) : après expiration, ré-authentification silencieuse (pas de 401) ---")
    mock_ttl(20); inv(m, "ionapi.admin:testAlias", {"alias": "demo"})
    time.sleep(17)
    r = inv(m, "ionapi.api:call", {"alias": "demo", "path": M3_PATH + "?CUNO=C000044"}, 200); assert r and r.get("status") == "200" and r.get("tokenSource") in ("password", "refresh") and r.get("retried") == "false", "ré-authentification KO"
    print("--- révocation côté serveur : 401 puis nouvel essai transparent (retried=true) ---")
    ...
    r = inv(m, "ionapi.api:call", {"alias": "demo", "path": M3_PATH + "?CUNO=C000045"}, 200); assert r and r.get("status") == "200" and r.get("retried") == "true", "nouvel essai après 401 KO"
