#!/usr/bin/env python3
"""Package ApiAuth : authentification d'API universelle pour webMethods Integration Server, sans gestion manuelle
des jetons. Un profil = un alias + un type ; le flow métier appelle apiauth.api:call(alias, method, path, body).

Types de profil                 champs
  none                          baseUrl
  basic                         baseUrl, username, password
  apikey                        baseUrl, keyName, keyValue, keyIn (header|query), keyPrefix
  bearer   (jeton statique)     baseUrl, token, headerName (Authorization), prefix (Bearer)
  oauth2                        baseUrl, grant, tokenUrl, clientId, clientSecret, clientAuth (body|basic), scope, audience, resource
      ...
      grant refresh_token       + refreshToken (fourni une fois)
      grant authorization_code  + authUrl, redirectUri, pkce     (connexion depuis l'interface d'administration)
      grant jwt_bearer          + keyStoreAlias, keyAlias, jwtAlgorithm, jwtIssuer, jwtSubject, jwtAudience, jwtClaims (objet)
  commun (optionnel)            proxyAlias, keyStoreAlias/keyAlias (mTLS), trustStore, testPath, extraParams (objet), preset
...
DTN = "yyyyMMddHHmmss"
JWT_DT = "dd/MM/yyyy HH:mm:ss"      # seul format accepté par pub.jwt:generateSignedJWT
...
def base64_of_string(src, target):
    """chaîne -> base64 (sans retour à la ligne)."""
...
def defaulted(src, target, default):
    """copie puis valeur par défaut, en deux étapes MAP séparées (dans la même étape, le résultat est imprévisible)."""
...
def load_profile(fail_from="$parent"):
    return [store_get("apiauth.profile.%alias%", "/cfgJson;1;0"),
            branch("/cfgJson", ("$null", [exit_(fail_from, "FAILURE", "Profil d'accès inconnu : %alias%")])),
            json_parse("/cfgJson;1;0", "/cfg;2;0"), mapstep(delete("/cfgJson;1;0"))]

# ------------------------------------------------------------------ apiauth.store (coffre chiffré de l'IS)
...
], "Écrit une valeur dans le coffre de mots de passe sortants de l'IS (chiffré au repos).")
...
], "Lit une valeur du coffre (value absent si la clé n'existe pas).")
...
], "Supprime une clé du coffre (silencieux si absente).")
...
LOG_EVENT = service("apiauth.log:event", PKG, sig(field("alias"), field("event"), field("detail")), sig(), [
    ...
    branch_expr(("%len% > 20000", [mapstep(delete("/journal;2;0", "/newJson;1;0")), append_doc("/journal;2;0/events;2;1", "/ev;2;0"), json_string("/journal;2;0", "/newJson;1;0")]), comment="journal borné à 20 Ko"),
...
], "Journal des événements d'authentification (jetons obtenus, renouvelés, refusés, configuration) conservé dans le coffre.")
...
def http_token(basic):
    ...
    inp = [i for i in inp if not (i["type"] == "MAPCOPY" and i["from"] == i["to"])]   # les champs déjà dans le pipeline passent tels quels
    pre = []
    if basic:
        inp += [setv("/auth;2;0/type;1;0", "basic"), copy("/clientId;1;0", "/auth;2;0/user;1;0"), copy("/clientSecret;1;0", "/auth;2;0/pass;1;0")]
    else:
        pre = [mapstep(copy("/clientId;1;0", "/args;2;0/client_id;1;0"), copy("/clientSecret;1;0", "/args;2;0/client_secret;1;0"), comment="client dans le corps (ajouté avant la copie du record : écrire un enfant après une copie de record le remplace)")]
    return pre + [invoke("pub.client:http", inp=inp, out=[copy("/header;2;0/status;1;0", "/httpStatus;1;0"), copy("/body;2;0/bytes;3;0", "/respBytes;3;0"), HTTP_CLEAN],
                  comment="client authentifié " + ("par en-tête Basic (RFC 6749 §2.3.1)" if basic else "dans le corps (client_id / client_secret)"))]
...
TOKEN_REQUEST = service("apiauth.oauth:tokenRequest", PKG,
    ...
    branch("/clientAuth", ("basic", http_token(True)), ("$default", http_token(False)), comment="méthode d'authentification du client"),
...
], "POST form-urlencoded vers le serveur de jetons (client dans le corps ou en Basic, proxy / mTLS optionnels) -> httpStatus, respJson.")

def token_request(args_nodes, comment):
    """args_nodes : MAPSET/MAPCOPY vers /args;2;0/... ; ajoute extraParams du profil, appelle tokenRequest."""
    return [mapstep(copy("/cfg;2;0/extraParams;2;0", "/args;2;0"), comment="paramètres supplémentaires du profil (copiés seuls : étape séparée)"), mapstep(*args_nodes, comment=comment),
            invoke("apiauth.oauth:tokenRequest", inp=[copy("/cfg;2;0/tokenUrl;1;0", "/tokenUrl;1;0"), copy("/cfg;2;0/clientId;1;0", "/clientId;1;0"), copy("/cfg;2;0/clientSecret;1;0", "/clientSecret;1;0"), copy("/cfg;2;0/clientAuth;1;0", "/clientAuth;1;0"),
                   ...
                   comment="les copies de la map d'entrée restent dans le pipeline de l'appelant : purge en sortie")]
...
def take_token(source):
    return [json_parse("/respJson;1;0", "/resp;2;0"),
            ...
            branch("/expObj", ("$null", []), ("$default", [invoke("pub.string:objectToString", inp=[copy("/expObj;3;0", "/object;3;0")], out=[copy("/string;1;0", "/expiresInRaw;1;0"), delete("/string;1;0", "/object;3;0")])]),
                   comment="expires_in : nombre ou chaîne selon les serveurs, 3600 s par défaut"),
...
STORE_TOKEN = service("apiauth.oauth:storeToken", PKG, sig(field("alias"), field("newToken"), field("newRefresh"), field("expiresInRaw"), field("tokenSource"), field("tokenType")), sig(field("expiresAt"), field("expiresIn")), [
    now_str("/now;1;0"),
    branch_expr(("%expiresInRaw% < 60", [mapstep(setv("/margin;1;0", "5"))]), ("$default", [mapstep(setv("/margin;1;0", "30"))]), comment="marge de sécurité : 30 s, ou 5 s pour les jetons courts"),
    ...
    log_event("%tokenSource%", "jeton obtenu, valable %expiresInRaw% s"),
...
], "Mémorise un jeton (chiffré) avec son échéance calculée localement, et journalise.")

# ------------------------------------------------------------------ apiauth.oauth:token
def jwt_assertion():
    """Assertion JWT signée par une clé du keystore de l'IS (Google, Salesforce, Box, Adobe...)."""
    return [now_str("/jwtNow;1;0"), mapstep(setv("/jwtTtl;1;0", "300")), add_seconds("/jwtNow;1;0", "/jwtTtl;1;0", "/jwtExp;1;0", JWT_DT),
            mapstep(copy("/cfg;2;0/jwtAudience;1;0", "/audItem;1;0"), copy("/cfg;2;0/tokenUrl;1;0", "/audDefault;1;0"), comment="audience : celle du profil, sinon l'URL du serveur de jetons"),
            ...
            branch("/cfg/jwtClaims", ("$null", []), ("$default", [
                ...
                mapstep(delete("/claimList;2;1"))]), comment="claims supplémentaires du profil (ex. scope pour Google)"),
            *defaulted("/cfg;2;0/jwtAlgorithm;1;0", "/jwtAlg;1;0", "RS256"),
            mapstep(copy("/cfg;2;0/clientId;1;0", "/jwtIss;1;0"), comment="issuer : celui du profil, sinon le client id"),
            ...
            invoke("pub.jwt:generateSignedJWT", inp=[copy("/jwtAlg;1;0", "/algorithm;1;0"), copy("/cfg;2;0/keyStoreAlias;1;0", "/keyStoreAlias;1;0"), copy("/cfg;2;0/keyAlias;1;0", "/keyAlias;1;0"),
                   ...
                   comment="signature RS256 par la clé privée du keystore de l'IS : la clé ne sort jamais"),
...
TOKEN = service("apiauth.oauth:token", PKG, sig(field("alias"), field("forceRefresh")), sig(field("accessToken"), field("tokenType"), field("tokenSource"), field("expiresAt"), field("expiresIn")), [
    ...
    *load_profile(),
    ...
    branch("/tokJson", ("$null", []), ("$default", [
        ...
        branch_expr(("%forceRefresh% != \"true\" && %tok/expiresAtNum% > %nowNum%", [mapstep(setv("/valid;1;0", "true")), secs_remaining("/now;1;0", "/tok;2;0/expiresAt;1;0", "/remaining;1;0")]), comment="jeton en cache encore valable ?")])),
    branch("/valid",
        ...
        ("$default", [
            mapstep(copy("/cfg;2;0/refreshToken;1;0", "/rtok;1;0"), comment="refresh token connu : celui du dernier jeton, sinon celui fourni dans le profil"),
            ...
            branch("/rtok", ("$null", []), ("$default", [
                ...
                quiet(*token_request([setv("/args;2;0/grant_type;1;0", "refresh_token"), copy("/rtok;1;0", "/args;2;0/refresh_token;1;0"), copy("/cfg;2;0/scope;1;0", "/args;2;0/scope;1;0")], "grant refresh_token"),
                      branch("/httpStatus", ("200", take_token("refresh")), ("$default", [log_event("refresh-ko", "refresh refusé (HTTP %httpStatus%) : on repasse par le grant principal")])),
                      mapstep(delete("/respJson;1;0", "/httpStatus;1;0")), comment="renouvellement, échec toléré")])),
            branch("/newToken", ("$null", [
                branch("/cfg/grant",
                    ("client_credentials", token_request([setv("/args;2;0/grant_type;1;0", "client_credentials"), copy("/cfg;2;0/scope;1;0", "/args;2;0/scope;1;0"), copy("/cfg;2;0/audience;1;0", "/args;2;0/audience;1;0"), copy("/cfg;2;0/resource;1;0", "/args;2;0/resource;1;0")], "grant client_credentials")),
                    ("password", token_request([setv("/args;2;0/grant_type;1;0", "password"), copy("/cfg;2;0/username;1;0", "/args;2;0/username;1;0"), copy("/cfg;2;0/password;1;0", "/args;2;0/password;1;0"), copy("/cfg;2;0/scope;1;0", "/args;2;0/scope;1;0"), copy("/cfg;2;0/audience;1;0", "/args;2;0/audience;1;0")], "grant password (compte de service)")),
                    ("jwt_bearer", [*jwt_assertion(), *token_request([setv("/args;2;0/grant_type;1;0", "urn:ietf:params:oauth:grant-type:jwt-bearer"), copy("/assertion;1;0", "/args;2;0/assertion;1;0"), copy("/cfg;2;0/scope;1;0", "/args;2;0/scope;1;0")], "grant jwt-bearer (RFC 7523)"), mapstep(delete("/assertion;1;0"))]),
                    ("refresh_token", [exit_("$parent", "FAILURE", "Aucun refresh token valable pour %alias% : fournissez-en un nouveau dans le profil")]),
                    ("authorization_code", [exit_("$parent", "FAILURE", "Connexion requise pour %alias% : utilisez « Connecter » dans l'interface d'administration (le refresh token est absent ou n'est plus accepté)")]),
                    ("$default", [exit_("$parent", "FAILURE", "Grant OAuth 2 inconnu : %cfg/grant%")]), comment="grant principal du profil"),
                branch("/httpStatus", ("200", take_token("%cfg/grant%")), ("$default", [exit_("$parent", "FAILURE", "Authentification refusée par le serveur de jetons (HTTP %httpStatus%) : %respJson%")])),
                mapstep(delete("/respJson;1;0", "/httpStatus;1;0"))]), comment="pas de jeton via refresh : grant principal"),
            branch("/newToken", ("$null", [exit_("$parent", "FAILURE", "Le serveur de jetons n'a pas renvoyé d'access_token")])),
            invoke("apiauth.oauth:storeToken", comment="mémorisation chiffrée + journal"),
            mapstep(copy("/newToken;1;0", "/accessToken;1;0"), delete("/newToken;1;0", "/newRefresh;1;0", "/expiresInRaw;1;0", "/rtok;1;0")),
        ]), comment="cache valable ? sinon refresh, sinon grant principal"),
    mapstep(delete("/cfg;2;0", "/tok;2;0", "/tokJson;1;0", "/now;1;0", "/nowNum;1;0", "/valid;1;0", "/remaining;1;0", "/forceRefresh;1;0")),
        ...
        mapstep(delete("/cfg;2;0", "/tok;2;0", "/tokJson;1;0", "/cfgJson;1;0", "/now;1;0", "/nowNum;1;0", "/valid;1;0", "/remaining;1;0", "/forceRefresh;1;0", "/newToken;1;0", "/newRefresh;1;0", "/rtok;1;0", "/expiresInRaw;1;0",
                       "/respJson;1;0", "/respBytes;3;0", "/httpStatus;1;0", "/resp;2;0", "/args;2;0", "/assertion;1;0", "/audience;1;1", "/customClaims;2;1"), comment="aucun secret ne doit rester dans le pipeline"),
        exit_("$flow", "FAILURE", "%tokenError%"),
    ]),
], "Jeton OAuth 2 valable pour un profil : cache chiffré, sinon refresh token, sinon grant principal (client_credentials, password, jwt_bearer). Les identifiants ne sortent jamais du service.")
...
AUTHORIZE_URL = service("apiauth.oauth:authorizeUrl", PKG, sig(field("alias"), field("redirectUri")), sig(field("url"), field("state")), [
    *load_profile("$flow"),
    branch("/cfg/authUrl", ("$null", [exit_("$flow", "FAILURE", "Le profil %alias% n'a pas d'URL d'autorisation (authUrl)")])),
    mapstep(copy("/cfg;2;0/redirectUri;1;0", "/redirectUriCfg;1;0"), comment="redirectUri : celui du profil, sauf si fourni en entrée"),
    ...
    mapstep(setv("/verifier;1;0", "%v1%%v2%", variables=True), delete("/v1;1;0", "/v2;1;0"), comment="PKCE : code_verifier aléatoire (72 caractères)"),
    ...
    branch("/cfg/pkce", ("false", []), ("$default", [mapstep(setv("/url;1;0", "%url%&code_challenge=%challenge%&code_challenge_method=S256", variables=True))]), comment="PKCE activé sauf refus explicite"),
    ...
    branch("/cfg/authExtra", ("$null", []), ("$default", [mapstep(setv("/url;1;0", "%url%&%cfg/authExtra%", variables=True))]), comment="paramètres supplémentaires bruts (ex. access_type=offline&prompt=consent pour Google)"),
...
], "Construit l'URL d'autorisation (authorization code + PKCE S256) et mémorise l'état de la demande.")
...
CALLBACK = service("apiauth.oauth:callback", PKG, sig(field("code"), field("state"), field("error"), field("error_description")), sig(), [
    *try_catch([
        branch("/state", ("$null", [exit_("$parent", "FAILURE", "Paramètre state absent")])),
        store_get("apiauth.pkce.%state%", "/pkJson;1;0"),
        branch("/pkJson", ("$null", [exit_("$parent", "FAILURE", "Demande d'autorisation inconnue ou déjà utilisée (state)")])),
        store_remove("apiauth.pkce.%state%"),
        json_parse("/pkJson;1;0", "/pk;2;0"), mapstep(copy("/pk;2;0/alias;1;0", "/alias;1;0")),
        branch("/error", ("$null", []), ("$default", [log_event("error", "autorisation refusée : %error% %error_description%"), exit_("$parent", "FAILURE", "Autorisation refusée par le serveur : %error% %error_description%")])),
        *load_profile(),
        *token_request([setv("/args;2;0/grant_type;1;0", "authorization_code"), copy("/code;1;0", "/args;2;0/code;1;0"), copy("/pk;2;0/redirectUri;1;0", "/args;2;0/redirect_uri;1;0"), copy("/pk;2;0/verifier;1;0", "/args;2;0/code_verifier;1;0")], "échange du code contre les jetons (PKCE)"),
        branch("/httpStatus", ("200", take_token("authorization_code")), ("$default", [exit_("$parent", "FAILURE", "Échange du code refusé (HTTP %httpStatus%) : %respJson%")])),
        branch("/newToken", ("$null", [exit_("$parent", "FAILURE", "Pas d'access_token dans la réponse")])),
        invoke("apiauth.oauth:storeToken", comment="mémorisation chiffrée + journal"),
        mapstep(setv("/html;1;0", CALLBACK_HTML.replace("%icon%", "✅").replace("%title%", "Connexion établie").replace("%detail%", "Le profil %alias% dispose maintenant d'un jeton (valable %expiresIn% s) et d'un refresh token. Cette fenêtre se ferme toute seule.").replace("%outcome%", "connected"), variables=True)),
        ...
        mapstep(setv("/alias;1;0", "?", overwrite=False), setv("/html;1;0", CALLBACK_HTML.replace("%icon%", "⛔").replace("%title%", "Connexion impossible").replace("%detail%", "%cbError%").replace("%outcome%", "failed"), variables=True)),
        respond("/html;1;0", "text/html; charset=utf-8"),
...
], "Point de retour du navigateur après consentement : échange le code (PKCE) contre les jetons, les mémorise et ferme la fenêtre.")
...
CREDENTIALS = service("apiauth.credentials:get", PKG, sig(field("alias"), field("forceRefresh")),
    ...
    *load_profile("$flow"),
    ...
    branch("/cfg/type",
        ...
        ("$default", [exit_("$flow", "FAILURE", "Type de profil inconnu : %cfg/type%")]), comment="un seul point d'entrée, quel que soit le mécanisme"),
    mapstep(copy("/cfg;2;0/keyStoreAlias;1;0", "/keyStoreAlias;1;0"), copy("/cfg;2;0/keyAlias;1;0", "/keyAlias;1;0"), delete("/cfg;2;0", "/forceRefresh;1;0"), comment="mTLS éventuel (certificat client du keystore)"),
], "Rend ce qu'il faut ajouter à une requête pour un profil : en-tête (Basic, clé, Bearer statique ou OAuth 2 géré) ou paramètre de requête. Le flow appelant ne voit jamais de secret brut.")
...
def build_headers():
    return [mapstep(setv("/h;2;0/name;1;0", "Accept"), setv("/h;2;0/value;1;0", "application/json")), append_doc("/hdrList;2;1", "/h;2;0"), mapstep(delete("/h;2;0"), comment="la liste référence le document : on repart d'un document neuf"),
            branch("/body", ("$null", []), ("$default", [mapstep(setv("/h;2;0/name;1;0", "Content-Type"), copy("/contentType;1;0", "/h;2;0/value;1;0")), append_doc("/hdrList;2;1", "/h;2;0"), mapstep(delete("/h;2;0"))]), comment="Content-Type seulement avec un corps"),
            branch("/headers", ("$null", []), ("$default", [
                ...
                loop("/userHdrs", None, append_doc("/hdrList;2;1", "/userHdrs;2;0")), mapstep(delete("/userHdrs;2;1", "/headers;2;0"))]), comment="en-têtes fournis par l'appelant"),
            branch("/authHeaderName", ("$null", []), ("$default", [mapstep(copy("/authHeaderName;1;0", "/h;2;0/name;1;0"), copy("/authHeaderValue;1;0", "/h;2;0/value;1;0")), append_doc("/hdrList;2;1", "/h;2;0"), mapstep(delete("/h;2;0"))]), comment="en-tête d'authentification (nom dynamique)"),
...
API_CALL = service("apiauth.api:call", PKG, sig(field("alias"), field("method"), field("path"), field("body"), field("contentType"), field("headers", "record")),
    ...
    invoke("apiauth.credentials:get", out=[copy("/type;1;0", "/authType;1;0"), delete("/type;1;0", "/expiresIn;1;0")], comment="identifiants du profil (jeton géré automatiquement)"),
    ...
    branch("/queryName", ("$null", []), ("$default", [
        ...
        mapstep(setv("/url;1;0", "%url%%sep%%queryName%=%eq%", variables=True), delete("/qpos;1;0", "/sep;1;0", "/eq;1;0"))]), comment="clé d'API en paramètre de requête"),
    *build_headers(),
    http_api_call("appel de l'API"),
    body_to_string("/respBody;1;0"), mapstep(delete("/respBytes;3;0")),
    branch_expr(("%httpStatus% == \"401\" && %authType% == \"oauth2\"", [
        invoke("apiauth.credentials:get", inp=[setv("/forceRefresh;1;0", "true")], out=[delete("/type;1;0", "/expiresIn;1;0", "/forceRefresh;1;0")], comment="401 : jeton révoqué côté serveur, on se ré-authentifie"),
        mapstep(setv("/retried;1;0", "true"), delete("/hdrs;2;0", "/respBody;1;0")),
        log_event("retry-401", "%method% %path% : 401, nouveau jeton puis nouvel essai"),
        *build_headers(), http_api_call("nouvel essai avec le nouveau jeton"), body_to_string("/respBody;1;0"), mapstep(delete("/respBytes;3;0"))]), comment="un seul nouvel essai sur 401 (OAuth 2 uniquement)"),
    quiet(json_parse("/respBody;1;0", "/json;2;0"), comment="corps JSON décodé si possible"),
...
], "Appelle une API avec un profil : l'authentification (Basic, clé, Bearer, OAuth 2 avec renouvellement) est appliquée automatiquement ; nouvel essai unique après un 401 OAuth 2.")

# ------------------------------------------------------------------ apiauth.admin
REQUIRED = {  # type -> (expression de champs manquants, message)
    "basic": ("%p/username% == $null || %p/password% == $null", "Basic : username et password sont requis"),
    "apikey": ("%p/keyValue% == $null", "Clé d'API : keyValue est requis"),
    "bearer": ("%p/token% == $null", "Jeton statique : token est requis"),
}
GRANT_REQUIRED = {
    "client_credentials": ("%p/tokenUrl% == $null || %p/clientId% == $null || %p/clientSecret% == $null", "client_credentials : tokenUrl, clientId et clientSecret sont requis"),
    "password": ("%p/tokenUrl% == $null || %p/clientId% == $null || %p/username% == $null || %p/password% == $null", "password : tokenUrl, clientId, username et password sont requis (clientSecret selon le serveur)"),
    "refresh_token": ("%p/tokenUrl% == $null || %p/clientId% == $null || %p/refreshToken% == $null", "refresh_token : tokenUrl, clientId et refreshToken sont requis"),
    "authorization_code": ("%p/tokenUrl% == $null || %p/authUrl% == $null || %p/clientId% == $null || %p/redirectUri% == $null", "authorization_code : tokenUrl, authUrl, clientId et redirectUri sont requis"),
    "jwt_bearer": ("%p/tokenUrl% == $null || %p/clientId% == $null || %p/keyStoreAlias% == $null || %p/keyAlias% == $null", "jwt_bearer : tokenUrl, clientId (issuer), keyStoreAlias et keyAlias sont requis"),
}
SECRET_FIELDS = ("password", "clientSecret", "keyValue", "token", "refreshToken")

def strip_empty():
    """Les chaînes vides envoyées par un formulaire valent « absent »."""
    nodes = []
...
SAVE_PROFILE = service("apiauth.admin:saveProfile", PKG, sig(field("profileJson"), field("keepToken")), sig(field("ok"), field("message"), field("alias"), field("type")), [
    branch("/profileJson", ("$null", [exit_("$flow", "FAILURE", "profileJson est requis")])),
    json_parse("/profileJson;1;0", "/p;2;0"),
    *strip_empty(),
    branch_expr(("%p/alias% == $null || %p/alias% == \"\"", [exit_("$flow", "FAILURE", "Le nom du profil (alias) est requis")]), ("%p/type% == $null", [exit_("$flow", "FAILURE", "Le type du profil est requis (none, basic, apikey, bearer, oauth2)")])),
    branch("/p/type",
        ("none", []),
        *[(t, [branch_expr((expr, [exit_("$flow", "FAILURE", msg)]))]) for t, (expr, msg) in REQUIRED.items()],
        ("oauth2", [branch("/p/grant", *[(g, [branch_expr((expr, [exit_("$flow", "FAILURE", msg)]))]) for g, (expr, msg) in GRANT_REQUIRED.items()],
                           ("$default", [exit_("$flow", "FAILURE", "Grant OAuth 2 requis : client_credentials, password, refresh_token, authorization_code ou jwt_bearer")]))]),
        ("$default", [exit_("$flow", "FAILURE", "Type de profil inconnu : %p/type%")]), comment="validation par type / grant"),
    ...
    branch("/oldJson", ("$null", []), ("$default", [json_parse("/oldJson;1;0", "/old;2;0"),
        *[branch(f"/p/{f}", ("$null", [mapstep(copy(f"/old;2;0/{f};1;0", f"/p;2;0/{f};1;0"))])) for f in SECRET_FIELDS],
        mapstep(delete("/old;2;0"))]), comment="modification : un secret absent conserve la valeur déjà dans le coffre"),
    ...
    log_event("config", "profil enregistré (type %type%)"),
    mapstep(setv("/ok;1;0", "true"), setv("/message;1;0", "Profil enregistré ; les secrets sont chiffrés dans le coffre de l'Integration Server"), delete("/p;2;0", "/cfgJson;1;0", "/profileJson;1;0", "/savedAt;1;0", "/keepToken;1;0", "/oldJson;1;0")),
], "Enregistre (ou remplace) un profil d'accès à partir de son JSON ; validation par type et grant ; secrets chiffrés dans le coffre.")

DELETE_PROFILE = service("apiauth.admin:deleteProfile", PKG, sig(field("alias")), sig(field("ok")), [
    store_remove("apiauth.profile.%alias%"), store_remove("apiauth.token.%alias%"), log_event("config", "profil supprimé"), mapstep(setv("/ok;1;0", "true")),
], "Supprime un profil et son jeton.")

DISCONNECT = service("apiauth.admin:disconnect", PKG, sig(field("alias")), sig(field("ok")), [
    store_remove("apiauth.token.%alias%"), log_event("config", "jeton oublié (déconnexion)"), mapstep(setv("/ok;1;0", "true")),
], "Oublie le jeton (et le refresh token) d'un profil sans toucher à sa configuration.")
...
LIST_PROFILES = service("apiauth.admin:listProfiles", PKG, sig(), sig(field("serverTime"), field("profiles", "record", 1), field("events", "record", 1)), [
    ...
    loop("/allKeys", None,
         ...
         branch("/pos", ("0", [
             ...
             mapstep(*[copy(f"/cfg;2;0/{f};1;0", f"/entry;2;0/{f};1;0") for f in PUBLIC_FIELDS], copy("/cfg;2;0/extraParams;2;0", "/entry;2;0/extraParams;2;0"), copy("/cfg;2;0/jwtClaims;2;0", "/entry;2;0/jwtClaims;2;0"), comment="jamais les secrets"),
         ...
         mapstep(delete("/pos;1;0")), comment="une entrée par clé apiauth.profile.*"),
    store_get("apiauth.events", "/evJson;1;0"),
...
], "Liste les profils (sans les secrets, avec indicateurs de présence), l'état de leur jeton et le journal.")
...
TEST_PROFILE = service("apiauth.admin:testProfile", PKG, sig(field("alias")), sig(field("ok"), field("message"), field("source"), field("expiresIn"), field("status"), field("elapsedMs")), [
    t0(),
    *try_catch([
        invoke("apiauth.credentials:get", inp=[setv("/forceRefresh;1;0", "true")], out=[delete("/forceRefresh;1;0")], comment="identifiants (jeton forcé pour OAuth 2)"),
        branch("/type", ("oauth2", [mapstep(setv("/message;1;0", "Jeton obtenu (%source%), valable %expiresIn% s", variables=True))]),
                        ("none", [mapstep(setv("/message;1;0", "Aucune authentification"))]),
                        ("$default", [mapstep(setv("/message;1;0", "Identifiants prêts (%type%)", variables=True))])),
        *load_profile(),
        branch("/cfg/testPath", ("$null", []), ("$default", [
            invoke("apiauth.api:call", inp=[copy("/cfg;2;0/testPath;1;0", "/path;1;0"), setv("/method;1;0", "get")],
                   out=[delete("/statusMessage;1;0", "/body;1;0", "/json;2;0", "/authType;1;0", "/retried;1;0", "/elapsedMs;1;0", "/url;1;0", "/path;1;0", "/method;1;0")], comment="appel de vérification (GET testPath)"),
            branch_expr(("%status% >= 200 && %status% < 300", [mapstep(setv("/message;1;0", "%message% ; GET %cfg/testPath% : HTTP %status%", variables=True))]),
                        ("$default", [exit_("$parent", "FAILURE", "%message% ; mais GET %cfg/testPath% répond HTTP %status%")]))]), comment="test d'appel si le profil définit testPath"),
    ...
    *elapsed_ms("/t0;3;0", "/elapsedMs;1;0"),
], "Teste un profil : obtient les identifiants (jeton forcé pour OAuth 2) puis, si testPath est défini, un GET de vérification.")
...
# ------------------------------------------------------------------ tests contre mock/auth_mock.py
...
def call(m, alias, path, expect="200", **kw):
    ...
    assert r and r.get("status") == expect, f"{alias}: attendu HTTP {expect}"
    return r

def tests(m):
    try: mock("/admin/ttl")
    except Exception: sys.exit("simulateur absent : python3 mock/auth_mock.py 8086 &")
    ...
    print("--- profils : basic, apikey (en-tête et requête), bearer statique, none ---")
    ...
    r = call(m, "t-basic", "/api/apikey/customers/C6", expect="401"); assert r.get("retried") == "false", "pas de nouvel essai hors OAuth 2"
    print("--- OAuth 2 client_credentials (client dans le corps puis en Basic) ---")
    ...
    r = inv(m, "apiauth.admin:testProfile", {"alias": "t-cc-basic"}); assert r and r.get("ok") == "true", "client Basic KO"
    print("--- OAuth 2 password (compte de service) + refresh + expiration + révocation ---")
    ...
    r = call(m, "t-pwd", "/api/bearer/customers/C10"); assert r.get("source") == "refresh" and r.get("retried") == "false", "renouvellement silencieux KO"
    mock("/admin/ttl", {"seconds": 90}); inv(m, "apiauth.admin:testProfile", {"alias": "t-pwd"}); mock("/admin/revoke", {})
    r = call(m, "t-pwd", "/api/bearer/customers/C11"); assert r.get("retried") == "true", "nouvel essai après 401 KO"
    print("--- OAuth 2 refresh_token fourni une fois (rotation) ---")
    ...
    mock("/admin/revoke", {}); r = call(m, "t-rt", "/api/bearer/customers/C13"); assert r.get("retried") == "true" and r.get("source") == "refresh", "rotation du refresh token KO"
    print("--- OAuth 2 jwt_bearer (assertion signée par la clé du keystore de l'IS) ---")
    ...
    print("--- OAuth 2 authorization_code + PKCE : URL, consentement simulé, callback ---")
    ...
    r = inv(m, "apiauth.api:call", {"alias": "t-ac", "path": "/api/bearer/customers/C15"}); assert r is None, "un profil non connecté doit échouer clairement"
    ...
    assert loc and "code=" in loc, "pas de redirection avec code"
    ...
    html = urllib.request.urlopen(rq, timeout=30).read().decode(); assert "Connexion établie" in html, html[:300]
    ...
    r = inv(m, "apiauth.admin:saveProfile", {"profileJson": json.dumps({"alias": "t-inc", "type": "oauth2", "grant": "client_credentials", "tokenUrl": ""})}); assert r is None, "profil incomplet accepté"
    save(m, alias="t-basic", type="basic", baseUrl=MOCK, username="basic-user", testPath="/api/basic/customers/T2")
    r = inv(m, "apiauth.admin:testProfile", {"alias": "t-basic"}); assert r and r.get("ok") == "true", "secret non conservé en modification"
    r = inv(m, "apiauth.admin:listProfiles", maxlen=400); assert r and not any(k in json.dumps(r) for k in ("basic-pass", "key-demo-123", "static-demo-token", CLIENT_SECRET)), "secret exposé dans la liste"
